...
Se o Auto Scaling Group adicionar um quarto servidor (10.0.0.4), o NGINX não saberá da existência dele. Se o servidor 10.0.0.1 morrer, o NGINX continuará enviando tráfego para ele (causando erros 502 Bad Gateway) até que uma pessoa edite o arquivo e recarregue o serviço.

Tentar resolver isso com scripts cron ou automações frágeis é um padrão ruim. Precisamos de uma solução que "assista" o estado do cluster e reaja imediatamente.
...
O benefício do Consul-Template é que ele não serve apenas para listas de IPs. Ele pode ser usado para injetar **Configurações Dinâmicas** vindas do Consul KV.
